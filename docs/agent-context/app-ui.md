...
- Hidden WebView2 surfaces must not keep compositing: `WebView2PowerSaver` suspends the Chromium renderer (visibility off, low memory target, `TrySuspendAsync`) for the idle overlay, for the main UI while the host window is minimized/hidden to tray, and while the minimal UI replaces `BlazorUI`. Resume happens before the surface is shown again (`BlazorOverlay.UpdateWindowVisibility`, `BlazorUI.OnOpen`, `PageSwitcher.OnStateChanged`). Keep this invariant when touching window show/hide or navigation paths.
