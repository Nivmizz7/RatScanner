...
internal static class OAuth2
{
    ...
    internal static async Task<Token?> DoOAuthAsync(Client client)
    {
        ...
        string authorizationRequest = string.Format(
            "{0}?response_type=code&scope={1}&redirect_uri={2}&client_id={3}&state={4}&code_challenge={5}&code_challenge_method=S256",
        ...
        ProcessStartInfo psi = new() { FileName = authorizationRequest, UseShellExecute = true };
        Process.Start(psi);
